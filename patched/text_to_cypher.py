...
class TextToCypherRetriever(BasePGRetriever):
    ...
    def retrieve_from_graph(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        if self.custom_cypher_query:
            response = self.custom_cypher_query
        else:
        ...
        self.generated_cypher_query = parsed_cypher_query

        if self.cypher_limit:
            parsed_cypher_query = parsed_cypher_query.replace(';', "")
            parsed_cypher_query = parsed_cypher_query.split("LIMIT")[0] + " LIMIT " + str(self.cypher_limit)
        ...
        self.query_output = cleaned_query_output
